...
function draw(rows) {
  ...
  const dateAt = index => { const date = new Date(start); date.setMonth(date.getMonth() + index); return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}`; };
  let grid = '';
  ...
  const base = rows.map((row, index) => `${x(index)},${y(row.contributed)}`).join(' '), top = rows.map((row, index) => `${x(index)},${y(row.total)}`).join(' '), area = `${base} ${x(rows.length - 1)},${y(0)} ${x(0)},${y(0)}`, every = 6;
